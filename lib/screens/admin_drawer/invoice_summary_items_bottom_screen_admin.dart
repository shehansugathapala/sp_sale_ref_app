...
class InvoiceSummaryItemsBottomSheetAdmin extends StatefulWidget {
  const InvoiceSummaryItemsBottomSheetAdmin({Key? key, required this.invoiceId, required this.refName}) : super(key: key);
  final String invoiceId;
  final String refName;

  @override
  _InvoiceSummaryItemsBottomSheetAdminState createState() => _InvoiceSummaryItemsBottomSheetAdminState();
}

class _InvoiceSummaryItemsBottomSheetAdminState extends State<InvoiceSummaryItemsBottomSheetAdmin> {
  ...
  void initState() {
    super.initState();
    Logger().i(widget.invoiceId.toString());
    _getInvoiceById(int.parse(widget.invoiceId.toString()));
  }

  Future<void> _getInvoiceById(int invoiceNumber) async {
    try {
      var variable = await FirebaseFirestore.instance.collection('invoice').where('id', isEqualTo: invoiceNumber).get();
  ...
  Future<List<InvoiceItems>> getProducts() async {
    return await FirebaseFirestore.instance
        ...
        .where('invoiceId', isEqualTo: widget.invoiceId + '_' + widget.refName)
        .get()
